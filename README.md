...
Лифт хранит в себе вызовы (этажи с нажатыми кнопками, на которые его отправил контроллер), нажатые внутри него кнопки вниз и вверх.

Его жизненный цикл: он ждет, пока ему назначат nextDestination, едет туда, сообщает этажу и контроллеру, что он приехал. 
	Тогда этаж сажает и высаживает людей. Если кто-то не влез, он снова дает людям нажимать кнопки. В лифт люди заходят в зависимости от поля willSitOnAnyElevator - если true, то человек, которому вверх, может сесть в лифт, едущий вниз. Иначе садится только в лифт с подходящим направлением (если задать что у всех людей это поле false, то по сути получится как будто с 2 очередями).
	Контроллер смотрит, есть ли еще лифты, у которых в вызовах есть этот этаж. Отменяет вызовы всем им (чтобы не получилось, что лифт приехал на пустой этаж). В любом случае затем меняет текущему лифту nextDestination.

Если лифту, когда он едет, назначают другой nextDestination, он едет туда (например, он едет с 1 на 10 этаж, на 5 этаже нажалась кнопка вверрх).

Возможные состояния лифта:
	1. EMPTY_STAND - он без пассажиров и без вызовов, стоит на каком-то этаже
	2. EMPTY_GO_TO_CALL - он без пассажиров, но едет на вызов (это состояние введено потому что пока люди не погрузятся в лифт, непонятно, поедет он дальше вниз или вверх). После того, как люди погрузились, лифт поедет на ближайшую нажатую кнопку (например, он посадил людей на 5 этаже, нажали кнопки на 2 и 6. Он поедет на 6 и станет GOIND_UP).
	3. GO_UP, 4. GO_DOWN - он уже с пассажирами, и теперь будет ехать в этом направлении, пока не закончатся нажатые кнопки или вызовы в этом направлении

Алгоритм выбора лифта по вызову:
	Выбирется из EMPTY_STAND и подходящего по направлению (GO_UP или GO_DOWN), тот, кто приедет быстрее всего
